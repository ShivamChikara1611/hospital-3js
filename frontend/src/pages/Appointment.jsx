...
const Appointment = () => {
    ...
    const texts = {
        en: {
            heading: "Book Your",
            headingSpan: "Appointment",
            headingEnd: "Now.",
            bookingSlots: "Booking Slots",
            appointmentFee: "Appointment fee:",
            bookButton: "Book",
            loginWarning: "Login to book an appointment"
        },
        jp: {
            heading: "予約する",
            headingSpan: "アポイントメント",
            headingEnd: "今すぐ",
            bookingSlots: "予約可能時間",
            appointmentFee: "予約料金:",
            bookButton: "予約する",
            loginWarning: "予約するにはログインしてください"
        }
    }
    ...
    useEffect(() => { getAvailableSlotes() }, [docInfo])
    ...
    return docInfo && (
        <div className='my-10 md:my-16 mx-2 md:mx-0'>
            <h1 className='text-gray-200 text-center tracking-wider text-2xl md:text-4xl mb-8 md:mb-12'>
                {t.heading} <span className='text-primary'>{t.headingSpan}</span> {t.headingEnd}
            </h1>
                ...
                <div className='relative bg-white/10 rounded flex flex-col items-center justify-center mx-auto lg:mx-0 lg:max-w-[380px]'>
                    ...
                    <div className='w-full flex items-center justify-between gap-3 p-2'>
                        ...
                        <div className='flex flex-col w-full'>
                            <p className='items-center text-xl text-gray-200'>{docInfo.name}</p>
                    ...
                    <div className='bg-primary/40 px-3 py-4 rounded-t-xl text-sm w-full'>
                        ...
                        <p className='text-gray-300 mt-1'>{t.appointmentFee} <span className='text-gray-100 tracking-widest text-md'>{currencySymbol}{docInfo.fees}</span></p>
                    </div>
                </div>

                {/* ---------------Booking Slot----------------------*/}
                <div className='mt-[60px] lg:mt-0'>
                    <p className='text-xl font-thin tracking-wide text-gray-200 text-center lg:hidden'>{t.bookingSlots}</p>

                    <div className='flex gap-2 w-full max-w-[470px] mx-auto justify-start overflow-x-auto mt-6 lg:mt-0'>
                        {docSlots.length && docSlots.map((item, index) => (
                            <div
                                onClick={() => setSlotIndex(index)}
                                className={`text-center py-3 w-[60px] shrink-0 rounded-b-xl tracking-wider rounded-t-none cursor-pointer ${slotIndex === index ? 'bg-gray-200 text-primary' : 'bg-white/10 text-gray-400 hover:bg-white/20 transition-all duration-200'}`}
                                key={index}
                            >
                    ...
                    <div className='flex gap-1 w-fit mx-auto max-w-[470px] justify-start overflow-x-scroll mt-4'>
                        {docSlots.length && docSlots[slotIndex].map((item, index) => (
                            <p
                                ...
                                className={`text-center py-1.5 text-xs font-light px-6 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-gray-200 text-primary' : 'bg-white/10 text-gray-400 hover:bg-white/20 transition-all duration-200'}`}
                                key={index}
                            >
                    ...
                    <div className='flex justify-center lg:mt-10'>
                        <button
                            ...
                            className='mt-6 bg-white/5 backdrop-blur-md text-gray-300 px-8 py-2 rounded-full hover:bg-primary/70 transition-all duration-300 font-light tracking-wider'
                        >
                            {t.bookButton}
                        </button>
                    </div>
