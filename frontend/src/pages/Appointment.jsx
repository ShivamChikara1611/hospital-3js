    ...
    return docInfo && (
        <div>
            <div className='mt-10 flex flex-col md:flex-row rounded-xl overflow-hidden max-w-[1200px] mx-auto'>
                <div className='bg-primary bg-opacity-60 flex items-end justify-center'>
                    <img className='md:scale-120 lg:scale-100' src={docInfo.image} alt="" />
                </div>

                <div className='flex flex-col bg-primary bg-opacity-10 py-4 px-6 w-full'>
                    <p className='flex items-center gap-2 text-2xl font-medium text-gray-400'>{docInfo.name} <img className='w-5' src={assets.verified_icon} alt=""/></p>
                    ...
                    <div>
                        <p className='flex items-center gap-1 text-sm font-medium text-gray-400 mt-3'>About</p>
                        <p className='text-sm text-gray-200 max-w-[700px] mt-1'>{docInfo.about}</p>
                    </div>

                    <p className='text-gray-400 font-medium mt-4'>Appointment fee: <span className='text-gray-300'>{currencySymbol}{docInfo.fees}</span></p>
                </div>
            </div>
            ...
            <div className='mt-[100px] max-w-[800px] mx-auto'>
                ...
                <div className='flex items-center gap-5 w-full overflow-x-scroll mt-6'>
                    {docSlots.length && docSlots[slotIndex].map((item,index)=>(
                        <p onClick={()=>setSlotTime(item.time)} className={`text-center py-1 w-[110px] min-w-[110px] border border-primary rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'bg-transparent text-primary hover:bg-primary hover:bg-opacity-20 hover:border-transparent hover:text-white transition-all duration-200'}`} key={index}>
