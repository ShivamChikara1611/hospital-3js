            ...
            <div className='flex items-center gap-4'>
                {
                    ...
                    ? <div className='flex items-center gap-2 cursor-pointer group relative'>
                        <img className='w-[55px] rounded-full bg-primary p-1' src={userData.image} alt=""/>
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden transition-all text-gray-300 backdrop-blur-md bg-opacity-80 bg-gray-900`}>
                    ...
                    <ul className='flex flex-col items-center gap-5 mt-12 px-5 text-lg font-medium w-full'>
                        ...
                        <a className='border border-primary rounded-full py-1 px-3 text-primary hover:text-gray-200 hover:bg-primary transition-all duration-300' href="http://localhost:5179/" target="_blank" rel="noopener noreferrer">Switch to Admin</a>
                    </ul>
                </div>
            </div>
        </div>
