...
const RelatedDoctors = ({ speciality, docId }) => {
    ...
    return (
        <div className='flex flex-col items-center max-w-[1200px] mx-auto my-16'>
            <h1 className='text-2xl font-light tracking-wide text-gray-200'>Top Doctors to Book.</h1>
            <p className='w-[250px] text-center text-sm font-light text-gray-400'>Simply browse through our extensive list of trusted doctors.</p>


            <div className='w-full grid grid-cols-2 sm:grid-cols-3 lg:grid-flow-cols-4 xl:grid-cols-6 gap-2 mt-8'>
                {relDoc.slice(0, 5).map((item, index) => (
                    <div onClick={() => { navigate(`/appointment/${item._id}`); scrollTo(0, 0) }} className='cursor-pointer overflow-hidden rounded-xl' key={index}>
